import { generateCourseCatalog } from './courseGenerator';
...
/**
 * Enhanced search function with better keyword matching for local data.
 */
const searchEnhancedCourses = (searchKeywords) => {
  const allCourses = generateEnhancedCourseCatalog();
  
  if (!searchKeywords || searchKeywords.trim() === '') {
    return allCourses.slice(0, 20);
  }

  const keywords = searchKeywords.toLowerCase().split(' ').filter(word => word.length > 2);

  const scoredCourses = allCourses.map(course => {
    let score = 0;
    const searchableText = `${course.title || ''} ${course.description || ''} ${course.category || ''} ${(course.skills || []).join(' ')}`.toLowerCase();
    
    keywords.forEach(keyword => {
      if ((course.title || '').toLowerCase().includes(keyword)) score += 50;
      if ((course.skills || []).some(skill => (skill || '').toLowerCase().includes(keyword))) score += 40;
      if ((course.category || '').toLowerCase().includes(keyword)) score += 30;
      if ((course.description || '').toLowerCase().includes(keyword)) score += 20;
    });
...
/**
 * The main exported function that orchestrates fetching and includes the fallback safety net.
 */
export const fetchCourses = async (searchKeywords = '', options = {}) => {
  ...
  if (allCourses.length < 10 && fallbackToGenerated) {
    ...
    const generatedCourses = searchEnhancedCourses(query, 30);
